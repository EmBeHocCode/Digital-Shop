...
export default function LogsPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<string | null>(null); // folder name to confirm
  ...
  const handleDeleteFolder = async (folder: string) => {
    if (confirmDelete !== folder) {
      ...
      setTimeout(() => setConfirmDelete((c) => c === folder ? null : c), 3000);
      return;
    }
    setConfirmDelete(null);
    setDeletingFolder(folder);
    try {
      await logsApiClient.deleteFolder(folder);
      toast.success(`Đã xóa ${folder}`);
      queryClient.invalidateQueries({ queryKey: ['log-folders'] });
    } catch {
  ...
  const handleDeleteOld = async (keepCount: number) => {
    const toDelete = (folders ?? []).slice(keepCount);
    if (toDelete.length === 0) { toast.info('Không có folder cũ để xóa'); return; }
    toast.loading(`Đang xóa ${toDelete.length} folder...`, { id: 'del-old' });
    let deleted = 0;
    for (const f of toDelete) {
      try { await logsApiClient.deleteFolder(f.name); deleted++; } catch { /* skip */ }
    }
    toast.dismiss('del-old');
    toast.success(`Đã xóa ${deleted} folder cũ`);
    queryClient.invalidateQueries({ queryKey: ['log-folders'] });
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium">Nhật ký hệ thống · {folders?.length ?? 0} phiên</p>
          </div>
        </div>
        {(folders?.length ?? 0) > 3 && (
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm"
              onClick={() => handleDeleteOld(5)}
              className="h-8 px-3 text-xs border-muted/40 text-muted-foreground hover:border-[#FF4B4B]/50 hover:text-[#FF4B4B]">
              <Trash2 className="h-3.5 w-3.5 mr-1.5" />Giữ 5 mới nhất
            </Button>
            <Button variant="outline" size="sm"
              onClick={() => handleDeleteOld(10)}
              className="h-8 px-3 text-xs border-muted/40 text-muted-foreground hover:border-[#FF4B4B]/50 hover:text-[#FF4B4B]">
              <Trash2 className="h-3.5 w-3.5 mr-1.5" />Giữ 10 mới nhất
            </Button>
          </div>
        )}
      </div>
        ...
        {foldersLoading ? (
          ...
          folders?.map((folder, index) => (
            <div
              ...
              className="group relative rounded-2xl border-2 border-border bg-card p-6 cursor-pointer hover:border-[#FF9600]/50 hover:shadow-lg transition-all duration-200 animate-slide-up"
              style={{ animationDelay: `${index * 30}ms` }}
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-lg font-semibold group-hover:text-[#FF9600] transition-colors flex-1 min-w-0 truncate">
                  {folder.name}
                </span>
              </div>
              {/* Delete button */}
                ...
                className={`absolute top-3 right-3 p-1.5 rounded-lg text-xs transition-all
                  ${
                    confirmDelete === folder.name
                      ? 'bg-[#FF4B4B] text-white opacity-100'
                      : 'opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-[#FF4B4B] hover:bg-[#FF4B4B]/10'
                  }`}
              >
                {deletingFolder === folder.name
                  ? <RefreshCw className="h-3.5 w-3.5 animate-spin" />
                  : confirmDelete === folder.name
                    ? <span className="px-0.5 font-semibold">Xác nhận?</span>
