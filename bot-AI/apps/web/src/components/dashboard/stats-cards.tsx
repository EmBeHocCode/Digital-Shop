...
export function StatsCards() {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Axios interceptor trả về botOffline:true thay vì throw khi bot offline
  ...
  // Bot offline — hiển thị placeholder thay vì lỗi đỏ
  if (error || !data) {
    return (
      <div className="rounded-2xl border-2 border-muted bg-muted/20 p-6">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-muted text-muted-foreground">
            <TrendingUp className="h-5 w-5" />
          </div>
          <div>
            <p className="font-semibold text-muted-foreground">Bot đang offline</p>
            <p className="text-xs text-muted-foreground/60">Bật bot để xem thống kê</p>
          </div>
        </div>
      </div>
    );
