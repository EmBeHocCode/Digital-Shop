...
function runMigrations(sqlite: Database) {
  ...
  // Tạo bảng game_accounts (số dư minigame theo user)
  sqlite.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_game_accounts_balance ON game_accounts(balance);
  `);

  // Tạo bảng stranger_auto_reply_state (trạng thái auto-reply người lạ)
  sqlite.exec(`
    CREATE TABLE IF NOT EXISTS stranger_auto_reply_state (
      user_id TEXT PRIMARY KEY,
      last_sent_at INTEGER NOT NULL,
      last_sent_date TEXT NOT NULL,
      sent_count_today INTEGER NOT NULL DEFAULT 1,
      updated_at INTEGER NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_stranger_reply_date ON stranger_auto_reply_state(last_sent_date);
  `);
