...
tasksApi.get('/', async (c) => {
  try {
    ...
    if (type && ['send_message'].includes(type)) {
      conditions.push(eq(agentTasks.type, type as 'send_message'));
    }

    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

    let listQuery = db.select().from(agentTasks);
    if (whereClause) {
      listQuery = listQuery.where(whereClause) as typeof listQuery;
    }

    const data = await listQuery.orderBy(desc(agentTasks.createdAt)).limit(limit).offset(offset);

    // Count total (áp dụng cùng filter để pagination chính xác)
    let countQuery = db.select({ count: count() }).from(agentTasks);
    if (whereClause) {
