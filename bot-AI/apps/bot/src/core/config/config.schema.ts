...
const CloudDebugSchema = z.object({
  ...
  matchAnyHash: z.boolean().default(false),
});

const DirectMessageAutoReplySchema = z.object({
  enabled: z.boolean().default(false),
  message: z
    .string()
    .default(
  ...
  cooldownMs: z.coerce.number().min(0).default(600000),
  dailyLimit: z.coerce.number().min(1).default(1),
});

const FriendDirectMessageReplyEnabledSchema = z.preprocess((value) => {
...
export const SleepModeSchema = z.object({
  ...
  checkIntervalMs: z.coerce.number().min(60000).default(60000), // Interval check (default 1 phút)
});
...
export const BotConfigSchema = z.object({
  ...
  strangerAutoReply: DirectMessageAutoReplySchema.optional().default({
    enabled: false,
    message: 'Em là bot tự động của anh Hùng. Hiện tại anh có thể đang bận, mong anh/chị chờ một chút ạ.',
    cooldownMs: 600000,
    dailyLimit: 1,
  }),
  friendDmReplyEnabled: FriendDirectMessageReplyEnabledSchema.optional().default(true),
  privateAutoReply: DirectMessageAutoReplySchema.optional().default({
    enabled: false,
...
export const LoggerConfigSchema = z.object({
  ...
  sessionIntervalHours: z.coerce.number().min(1).max(24).default(5),
  maxSessionFolders: z.coerce.number().min(1).default(100),
});
...
// Command Access / RBAC schema
export const CommandAccessConfigSchema = z.object({
  commandsEnabled: z.boolean().default(true),
  // true = command cũng bị ràng buộc bởi allowedUserIds/allowedGroupIds
  // false = command dùng cơ chế quyền riêng (admin/role/disabled)
  respectGlobalAccessLists: z.boolean().default(false),
  additionalAdminUserIds: z.array(z.string()).default([]),
  nonAdminAllowedCommands: z.array(z.string()).default([]),
  denyMessage: z.string().default('⛔ Bạn không có quyền sử dụng lệnh này.'),
  // Per-command role: 'everyone' (default) | 'admin'
  commandRoles: z.record(z.string(), z.enum(['everyone', 'admin'])).default({}),
  // Disabled commands: lệnh bị vô hiệu hóa (không ai dùng được)
  disabledCommands: z.array(z.string()).default([]),
});

// Profile schema - dùng cho web dashboard (không ảnh hưởng logic chat bot)
...
export const ProfilesSchema = z.object({
  admin: UserProfileSchema.optional().default({
    displayName: '',
    pronouns: '',
    bio: '',
    avatarUrl: '',
  ...
  bot: UserProfileSchema.optional().default({
    displayName: '',
    pronouns: '',
    bio: '',
    avatarUrl: '',
...
export const SettingsSchema = z.object({
  ...
  bot: BotConfigSchema.optional().default({
    ...
    allowNSFW: false,
    customSystemPrompt: '',
    cloudDebug: { enabled: false, prefix: '#bot', matchAnyHash: false },
    strangerAutoReply: {
      enabled: false,
      message: 'Em là bot tự động của anh Hùng. Hiện tại anh có thể đang bận, mong anh/chị chờ một chút ạ.',
      cooldownMs: 600000,
    ...
    sleepMode: { enabled: false, sleepHour: 23, wakeHour: 6, checkIntervalMs: 60000 },
  ...
  // true = bỏ qua whitelist nhóm (cho phép tất cả nhóm)
  // false = chỉ cho phép các nhóm trong allowedGroupIds (có thể rỗng = chặn toàn bộ)
  allowAllGroups: z.boolean().default(true),
  autoReplyGroupIds: z.array(z.string()).default([]), // Nhóm đang bật chế độ tự reply (không cần @mention)
  profiles: ProfilesSchema.optional().default({
    admin: {
      displayName: '',
      pronouns: '',
      bio: '',
  ...
  commandAccess: CommandAccessConfigSchema.optional().default({
  ...
  logger: LoggerConfigSchema.optional().default({
    ...
    sessionIntervalHours: 5,
    maxSessionFolders: 100,
  }),
...
export type CommandAccessConfig = z.infer<typeof CommandAccessConfigSchema>;
export type DirectMessageAutoReplyConfig = z.infer<typeof DirectMessageAutoReplySchema>;
export type StrangerAutoReplyConfig = DirectMessageAutoReplyConfig;
