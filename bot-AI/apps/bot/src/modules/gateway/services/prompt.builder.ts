...
export function buildPrompt(
  ...
  const normalizedUserText = normalizeSlang(userText || '');
  const effectiveUserText = normalizedUserText || userText;
  const chatStyleHint = buildChatStyleHint(userText || '');
  ...
  if (hasMedia && !quoteHasMedia) {
    ...
    prompt = effectiveUserText || '(người dùng không nhập text)';
    const quoteText =
  ...
  if (youtubeUrls.length > 0) {
    if (hasMedia) {
      ...
      prompt = PROMPTS.youtube(youtubeUrls, effectiveUserText);
    }
  }

  const senderContext = `\n[SENDER CONTEXT]
- SenderID: ${senderId || '(không rõ)'}
- SenderName: ${senderName || '(không rõ)'}
- IsGroup: ${isGroup ? 'true' : 'false'}
`;

  const chatStyleContext = chatStyleHint
    ? `\n[CHAT STYLE CONTEXT]
- RawUserText: ${userText || '(trống)'}
- NormalizedUserText: ${effectiveUserText || '(trống)'}
${chatStyleHint}
`
    : '';

  const realtimeContext = `\n${buildRealtimeTimeHolidayContext(userText || '')}\n`;

  return `${senderContext}${realtimeContext}${chatStyleContext}\n${prompt}`;
}
